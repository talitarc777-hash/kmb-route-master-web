...
- Planned `leave at` / `arrive by` searches load `/operator-data/kmb_operation_time_slots.runtime.json`
- The runtime file is generated by `scripts/analyze_kmb_operation_time_slots.py`
- The analyzer now auto-discovers `.jsonl` / `.csv` inputs in `KMB csv time slot`, including the latest uploaded `kmb_eta_observations*.jsonl` files
- The day classes are `weekday` (Mon-Fri), `saturday`, and `sunday_public_holiday` (Sunday plus 2026 HK public holidays)
- Full and compact review JSON files are still generated for inspection, but the app does not fetch them during search
- The runtime file keeps only route/route-stop lookup keys plus start/end minutes, sample count, and route-stop sample-day count
- The runtime file omits stop names, coordinates, active-slot arrays, and verbose metadata to reduce browser download/parse time
